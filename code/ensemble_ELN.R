...
test <- try(load(paste0(char,'_1_model_full.rda')),silent=T)
if(class(test)%in%'try-error') next
test <- vector()
flag <- vector()
for(i in 1:100){
  test <- try(load(paste0(char,'_',i,'_model_full.rda')),silent=T)
  if(class(test)%in%'try-error') next
  else flag <- c(flag,i) 
  
  try(load(paste0(char,'_',i,'_model_full.rda')),silent=T)
...
save(Accuracy_sum_ELN,model_sum,test_sum,Accuracy_sum_ensem_ELN,time_ELN,file=paste0(char,'_full_ensemble_model.rda'))
